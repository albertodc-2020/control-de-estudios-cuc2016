...

{
	...
	public function autenticar($usuario,$clave,$cedula,$pgconn)
	{
		$query="select * from login where  af_usuario='$usuario' and af_contrasena='$clave' and af_cedula='$cedula';";
		$consulta = pg_query($pgconn,$query) or die ("Consulta errónea:".pg_last_error());
		if ($consulta)
		{
			$columna =pg_fetch_array($consulta);
			$validacion1 = $columna["nu_nivel"];
			$validacion2 = $columna["nu_status"];
			$validacion3 =$columna["af_cedula"];
			
		switch(true)
		{
			case $validacion1 =="1" && $validacion2=="1"&& $validacion3="$cedula":
				 header('location:../vista/v_menu.html');
				 break;
			case $validacion1 =="2" && $validacion2=="1"&& $validacion3="$cedula" :
				header('location:../vista/v_menu2.html');
				break;
			case $validacion1 =="3" && $validacion2=="1"&& $validacion3="$cedula" :
				header('location:../vista/v_menu3.html');
				break;
			default:
				echo "Usuario ó Contraseña no Valida";
			}
		}
		else
		{
